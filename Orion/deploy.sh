#!/bin/bash
# Orion Deploy Script - Run on server via SSH
...
cp server.js /opt/Orion/Orion-server/server.js

echo "[5] Start Node.js API on 5188..."
cd /opt/Orion/Orion-server
node server.js > /opt/Orion/node_api.log 2>&1 &
...
cd /opt/Orion/Orion-server
node serve.js > /opt/Orion/serve.log 2>&1 &
