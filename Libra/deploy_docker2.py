#!/usr/bin/env python3
"""Build and deploy Libra via Docker"""
import paramiko
import os
import time
...
print("  Vue dist...")
n = upload_tree(sftp, os.path.join(LOCAL, 'Libra-web', 'dist'), REMOTE + '/Libra-web/dist')
print(f"  Vue dist: {n} files")

print("  Libra source...")
for f in ['Libra.py', 'Libra.db']:
...
rmkdir(sftp, REMOTE + '/Libra-server')
for f in ['app_batch.py', 'serve.py']:
    fp = os.path.join(LOCAL, 'Libra-server', f)
    if os.path.exists(fp):
        upload_file(sftp, fp, REMOTE + '/Libra-server/' + f)
        print(f"  {f}: OK")
...
out, err = run(
    f'docker run -d --name libra -p 5188:5188 -p 5189:3000 '
...
print("\n[*] Copying files to /opt/Libra (persistent)...")
run('mkdir -p /opt/Libra')
for item in ['Libra.py', 'Libra.db', 'Config', 'Framework', 'Moudle', 'ORM', 'Tools']:
    run(f'cp -r "{REMOTE}/{item}" "/opt/Libra/" 2>/dev/null || true')
run(f'cp -r "{REMOTE}/Libra-server" "/opt/Libra/" 2>/dev/null || true')
print("[+] /opt/Libra updated")
