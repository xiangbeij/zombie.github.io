<template>
  <div class="monitor-page">
    ...
    <div class="page-header">
      <div>
        <h2 class="page-title">🌡️ 网站状态监控</h2>
        <p class="page-desc">实时探测网站可达性、响应时间与 SSL 证书状态</p>
      </div>
    ...
    <div class="white-card">
      <el-table :data="filteredMonitors" stripe v-loading="loading" empty-text="暂无监控目标，点击上方添加">
        <!-- 状态指示器 -->
        ...
        <!-- 监控名称 & URL -->
        <el-table-column label="监控目标" min-width="200">
          <template #default="{ row }">
        ...
        <!-- SSL 状态 -->
        <el-table-column label="SSL 证书" width="150" align="center">
          <template #default="{ row }">
            ...
            <span v-else-if="row.ssl_valid === 0" class="ssl-bad">🔒 无效</span>
            <span v-else class="muted">非HTTPS</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="{ row }">
            ...
            <el-button text type="info" size="small" @click="openHistory(row)">📊</el-button>
            <el-button text type="primary" size="small" @click="handleEdit(row)">✏️</el-button>
            <el-button text type="danger" size="small" @click="handleDelete(row)">🗑️</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 添加/编辑监控弹窗 -->
    <el-dialog v-model="dialogFormVisible" :title="editingMonitor ? '✏️ 编辑监控' : '➕ 添加监控'" width="520px" destroy-on-close>
      <el-form :model="form" label-position="top">
        <el-form-item label="监控名称" required>
          <el-input v-model="form.name" placeholder="例如：青岛农大主页" clearable />
        </el-form-item>
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="handleSave" :loading="saving">
          {{ editingMonitor ? '保存修改' : '确认添加' }}
        </el-button>
      </template>
    </el-dialog>

    <!-- 检测历史弹窗 -->
    <el-dialog v-model="dialogHistoryVisible" :title="'📊 ' + (historyMonitor?.name || '') + ' 检测历史'" width="700px" destroy-on-close>
      <el-table :data="history" stripe size="small" max-height="400" v-loading="historyLoading" empty-text="暂无历史记录">
        <el-table-column label="状态" width="80" align="center">
          <template #default="{ row }">
        ...
        <el-table-column label="HTTP状态" width="90" align="center">
          <template #default="{ row }">
        ...
        <el-table-column label="SSL 剩余" width="100" align="center">
          <template #default="{ row }">
            <span v-if="row.ssl_days_left >= 0">{{ row.ssl_days_left }} 天</span>
            <span v-else class="muted">—</span>
          </template>
        </el-table-column>
        <el-table-column label="错误信息" min-width="150">
...
const fetchMonitors = () => {
  ...
  getSiteMonitors({ page: currentPage.value, page_size: pageSize, search: search.value })
    ...
    .catch(e => ElMessage.error('获取监控列表失败: ' + (e.message || '')))
...
const handleSave = () => {
  if (!form.value.name.trim()) { ElMessage.warning('请输入监控名称'); return }
  if (!form.value.url.trim()) { ElMessage.warning('请输入目标 URL'); return }
  saving.value = true
  ...
  op.then(() => {
    ElMessage.success(editingMonitor.value ? '监控已更新' : '监控已添加')
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确认删除监控 "${row.name}" ?`, '删除确认', { type: 'warning' }).then(() => {
...
const handleCheck = (row) => {
  ...
  checkSiteMonitor(row.id)
    .then(() => {
      ElMessage.success({ message: `${row.name}: ${row.url} 检测完成`, duration: 3000 })
      fetchMonitors()
      fetchStats()
    })
    .catch(e => ElMessage.error('检测失败: ' + (e.message || '')))
...
const handleCheckAll = () => {
  ...
  checkAllSiteMonitors()
    .then(r => {
      ElMessage.success({ message: `已完成 ${r.data.checked || 0} 个站点的检测`, duration: 3000 })
      fetchMonitors()
      fetchStats()
    })
