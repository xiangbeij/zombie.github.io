...
var (
	...
	baseDir       = flag.String("dir", "/opt/Libra", "Libra base directory")
...
func main() {
	...
	if *baseDir == "/opt/Libra" {
		if filepath.Base(filepath.Dir(binDir)) == "Libra-server" {
			*baseDir = filepath.Dir(filepath.Dir(binDir))
		} else if cwd, err := os.Getwd(); err == nil && filepath.Base(cwd) == "Libra-server" {
			*baseDir = filepath.Dir(cwd)
		}
	}

	log.Printf("[Libra] base_dir=%s bin=%s", *baseDir, binDir)
	...
	if *serveStatic {
		webRoot := filepath.Join(*baseDir, "Libra-web", "dist")
	...
	log.Printf("[Libra] Go API starting on %s | dir=%s | workers=%d", *port, *baseDir, *maxConcurrent)
