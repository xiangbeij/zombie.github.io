...
run('docker rm libra 2>/dev/null; echo done')

print('[*] Starting Docker container...')
result = run(f'docker run -d --name libra -p 5188:5188 -p 5189:3000 --restart unless-stopped -v {REMOTE}/reports:/app/reports libra:latest 2>&1')
...
print('    ' + run('docker ps --filter name=^libra$ --format "{{.Names}} {{.Status}}"').strip())

print('[*] Docker logs:')
logs = run('docker logs libra 2>&1 | head -20').strip()
